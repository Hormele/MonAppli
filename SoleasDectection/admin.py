...
from .models import Utilisateur, Dataset, ModeleML, Campagne, Notification,GestionnaireRisque, Suspect, RegleMetier
...
class DatasetAdmin(admin.ModelAdmin):
    list_display = ('nom', 'description', 'type', 'fichier', 'dateC', 'format_fichier')
    search_fields = ('nom', 'description')
    list_filter = ('type', 'fichier')
...
class GestionnaireRisqueAdmin(admin.ModelAdmin):
    list_display = ('id', 'campagne', 'date')
    search_fields = ('campagne__id',)
    list_filter = ('date', 'campagne')
    readonly_fields = ('date',)
# 7. ------------------------------------------------------------------
@admin.register(Suspect)
class SuspectAdmin(admin.ModelAdmin):
    list_display = ('nom', 'email', 'matricule', 'decision', 'campagne')
    search_fields = ('nom', 'email', 'matricule')
    list_filter = ('decision', 'campagne')
    formfield_overrides = {
